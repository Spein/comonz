...
export function getUserpaidContents() {
    ...
    if (transactions) {
        urls.forEach(async function(transaction) {
            ...
            if (status === 'paid') {
                let transactionDetails = await firebase.database().ref('/transactions/' + authorKey).once('value').then(function(data) {
                    return data.val()
                })
...
export async function getSendPaymentsbyAuthors() {
    ...
    filteredclientIds.forEach(async function(valeur, i) {
        let authorId = await firebase.database().ref('/transactions/' + valeur).once('value').then(function(data) { return data.val().authorId })
        let users = await firebase.database().ref('/users/' + authorId).once('value').then(function(data) { return data.val() })
...
export async function getReceivedPaymentsbyContents() {
    ...
    let transactions = await firebase.database().ref('/transactions/' + authorKey).once('value').then(function(data) { return data.val() })
...
export async function getReceivedPaymentsbyUsers() {
    ...
    var filteredclientIds = clientIds.filter((x, i, a) => a.indexOf(x) == i)
    filteredclientIds.forEach(async function(valeur, i) {
        let clientUser = await firebase.database().ref('/users/' + valeur).once('value').then(function(data) { return data.val() })
